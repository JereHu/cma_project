...
The data used for this analysis is cycling data recorded with the movement tracking app POSMO over multiple trips by the author in Switzerland between Würenlos and Altstetten (@fig-dataoverview). The GPS data is mostly recorded every 10 seconds and the collection was done on 8 days between April 7th and Mai 21st. The data has been pre-processed, so that only bike trips are shown and only points from bike trips were included. The data is verified, meaning that the GPS location are rather precise and no outliers were found visually. The data was also cropped, so that the home location cannot be exactly determined.
```{r}
...
knitr::kable(head(posmo_sf_cut))
```

To calculate the slope, a Digital Elevation Model (DEM) of Switzerland with a 0.5 meter resolution from swissALTI3D was used. The relevant tiles were hand-picked (@fig-handpicked) to cover roughly the same extent as the bike data . This resulted in 190 tiles of 1km x 1km.
...
```{r}
#| label: fig-height
#| fig-cap: Visualization of data points aggregated with height attribute from DEM.
...
The points that were determined to be stops where then excluded from the data.
```{r}
#| label: fig-speed
#| fig-cap: Visualization of data points aggregated with the speed attribute.
...
This led to the distribution presented on the right in @fig-hist, which looks normally distributed with not many outliers. This would be expected for continuous bike data.
...
```{r}
#| label: fig-gradient
#| fig-cap: Visualization of data points aggregated with the gradient attribute.
...
```{r}
#| label: fig-segmentup
#| fig-cap: Visualization of upwards of segments. (There are also downward segments.)
...
The results of the summary and the regression (as can be seen in the below and @fig-scatter) do not indicate a relationship between speed difference and uphill gradient. With an R-squared value of 0.02, there is no explanation of the variance in speed by the uphill gradient. 
...
```{r}
#| label: fig-scatter
#| fig-cap: Scatterplot of Segments that are upward with their gradient and speed-difference of the flat parts before and after.
