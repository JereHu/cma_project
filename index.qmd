...
To calculate the slope, a Digital Elevation Model (DEM) of Switzerland with a 0.5 meter resolution from swissALTI3D (@swisstopo2021) was used. The relevant tiles were hand-picked (@fig-handpicked) to cover roughly the same extent as the bike data . This resulted in 190 tiles of 1km x 1km.
...
The DEM height attribute was then extracted and added to the bike data points as can be seen in @fig-height.
...
The data was then filtered for points that represent stops in movement (like breaks). In @Laube2011, this was done by comparing average distance within the moving window. For this study, the stop criterion was adapted such that a cyclist stops when the average speed of the current, the previous and next point is less than 1 km/h, or if the distance covered from the last point or to the next point is less than 2 meters. This criterion was empirically tested on the data.
...
#### Gradient
To calculate the gradient of each point, the distance to the next point was divided by the height difference to that point and multiplied by 100 to get percent (see @fig-slopecalc), as is standard for road gradients. 
...
This led to the distribution presented on the right in @fig-hist, which looks normally distributed with not many outliers, opposed to the figure on the left in @fig-hist. This would be expected for continuous bike data. The gradient added to the data points can be seen in @fig-gradient
...
The results of the summary and the regression (as can be seen below and in @fig-scatter) do not indicate a relationship between speed difference and uphill gradient. With an R-squared value of 0.02 and a p-value of 0.24, there is no explanation of the variance in speed by the uphill gradient. 

To check if the method is flawed or if the results are influenced by the choice of gradient for ‘steepness’, two more runs were made, one with a gradient of 5% and with 10% (based roughly on @Winters2016 and @Duc2008).
...
Another issue is with how the experiment is at the moment, a flat part can be after a steep part and at the same time in front of a steep part. This means that if the cyclist is tired after the first steep part this shows in the speed difference of the first steep part, but then another hill comes and the cyclist might recover only after the second one, which results in the second steep part having a speed increase. When looking at the scatterplots (@fig-scatter, @fig-scathist5, @fig-scathist10) and the data frames, it seems possible that this happens fairly often. At least that is the most obvious explanation in my opinion.
...
One last issue is, that the author had to fulfill their civil duties and went to the military in the last few weeks of the project, which hindered its development.
