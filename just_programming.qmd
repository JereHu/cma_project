...
#leave out everything that's not bike data, filter the polygon, also leave out 4-11-2023 and 4-25-2023, because that was not a bike tour
posmo_sf_cut <- posmo_sf |> 
  filter(transport_mode == "Bike")%>%
  st_filter(poly, .predicate=not_covered_by) |> 
  ungroup() |> 
  filter(datetime > as.Date('2023-04-12') | datetime < as.Date('2023-04-10')) |> 
  filter(datetime > as.Date('2023-04-26') | datetime < as.Date('2023-04-24')) |> 
  mutate(
...
```{r}
for(i in 1:nrow(avg_speeds)) {       # for-loop over rows
  if (avg_speeds[i+1, ]$uphill & avg_speeds[i, ]$flat){
    avg_speeds[i, ]$after_steep = TRUE
  }
}
```
...
```{r}
test <- bike_gradient |> 
  filter(segment_id_down == 46 | segment_id_down == 47 | segment_id_down == 48)
```
